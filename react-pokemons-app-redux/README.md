...
Dans `src/app/store/slices.js` ajouter dans la section reducer `setFilter` qui prendra action.payload et l'affectera à `state.filter` (immer s'occupera de créer le nouvel state tout seul), puis exporter l'action creator `setFilter`

Dans `src/app/components/pokemon-search.jsx` utiliser useSelector et le selecteur `pokemonsFilterSelector` pour récupérer la valeur de filter et l'afficher dans le champ (anciennement le state term)

Dans `src/app/components/pokemon-search.jsx` récupérer dispatch avec useDispatch et à la saisie dans le champ et dispatcher l'action setFilter (avec la valeur saisie en param) 